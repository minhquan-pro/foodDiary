  ...
  async ({ page = 1, limit = 10 } = {}, { rejectWithValue }) => {
    try {
      const { data } = await api.get(
        `/posts/friends?page=${page}&limit=${limit}`
      );
      return data.data;
    } catch (err) {
      return rejectWithValue(err.message);
...
export const followFromFeed = createAsyncThunk(
  "feed/followFromFeed",
...
export const unfollowFromFeed = createAsyncThunk(
  "feed/unfollowFromFeed",
  async (userId, { rejectWithValue }) => {
    try {
...
export const fetchBlockedIds = createAsyncThunk(
  "feed/fetchBlockedIds",
  async (_, { rejectWithValue }) => {
    try {
...
export const blockFromFeed = createAsyncThunk(
  "feed/blockFromFeed",
  async (userId, { rejectWithValue }) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFeed.fulfilled, (state, action) => {
        ...
        if (pagination.page === 1) {
          ...
          state.likedPostIds = [
            ...new Set([...state.likedPostIds, ...likedPostIds]),
          ];
        }
    ...
    builder.addCase(blockFromFeed.fulfilled, (state, action) => {
      ...
      state.posts = state.posts.filter(
        (post) => post.user.id !== action.payload
      );
      state.followingIds = state.followingIds.filter(
        (id) => id !== action.payload
      );
